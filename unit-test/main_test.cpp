...
namespace foobar::tests {
	...
	TEST(informarpaciente, funcioninformar)
	{
		...
		EXPECT_TRUE(vincularcontacto(pac1,arch)==true);
	}
	TEST(buscarUnPaciente,funcionbusqueda){
		Paciente pac1;
		ifstream miArch;
		miArch.open("Paciente.csv");
		string _id = "10242589";//me copio un id exacto del archivo depaciente para fijarme si realmente encuentra ese paciente 
